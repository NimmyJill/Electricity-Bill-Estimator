...
const db =
  "mongodb+srv://user_nimmy:1Olipparambil@mycluster.7dk8q.azure.mongodb.net/ElectricityBillEstimator?retryWrites=true&w=majority";
...
router.get("/admin", function (req, res) {
  ...
  ConsumerData.find().then(function (consumers) {
...
router.post("/login", function (req, res) {
  ...
  Userdata.findOne({ consumerName: Uname, password: password }, (err, user) => {
    if (err) {
      console.log(err);
...
router.post("/singleConsumer", function (req, res) {
  ...
  ConsumerData.findOne({ _id: req.body.id }).then(function (consumers) {
    res.send(consumers);
  });
});
router.post("/updateConsumer", function (req, res) {
  ...
  ConsumerData.updateOne(
    { _id: req.body.consumer._id },
    {
      $set: {
...
router.post("/deleteConsumer", function (req, res) {
  ...
  console.log(id);
  ConsumerData.deleteOne({ _id: id }).then((data) => {
    res.send("Deleted");
  });
});
...
router.post("/bill", function (req, res) {
  ...
  ConsumerData.findOne({
    consumerName: req.body.data.consumerName,
    consumerNumber: req.body.data.consumerNumber,
