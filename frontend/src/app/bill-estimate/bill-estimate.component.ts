...
export class BillEstimateComponent implements OnInit {
  ...
  calcMonthlyCharge(monthlyUsage, connectionType):any
  {
    ...
    if (monthlyUsage>500){
        ...
        if (monthlyUsage-unitsAbove200-unitsAbove150-unitsAbove100 == 100){
            ...
            unitsAbove50 = (monthlyUsage-unitsAbove200-unitsAbove150-unitsAbove100)%50
        }
        unitsAbove0 = monthlyUsage-unitsAbove200-unitsAbove150-unitsAbove100-unitsAbove50

        monthlyUsageCharge = (unitsAbove200*7.6)+(unitsAbove150*6.4)+(unitsAbove100*4.8)+(unitsAbove50*3.7)+(unitsAbove0*3.15)
        if (unitsAbove200 != 0){
            if (connectionType == '1 Phase'){
 ...
 calculateBill(usedUnits, usageDays, connectionType):any
 {
   ...
   let forecastedBillPerOverUsage;
    ...
    monthlyOneUnitUnderUsage = (usedUnits+((dailyUsage-1)*(60-usageDays)))/2
    if (monthlyOneUnitUnderUsage<0){
        monthlyOneUnitUnderUsage = 0
    }
    monthlyOneUnitOverUsage = (usedUnits+((dailyUsage+1)*(60-usageDays)))/2
    
    let monthlyCharge = this.calcMonthlyCharge(monthlyUsage, connectionType)
    let monthlyUnderUsageCharge = this.calcMonthlyCharge(monthlyOneUnitUnderUsage, connectionType)
    let monthlyOverUsageCharge = this.calcMonthlyCharge(monthlyOneUnitOverUsage, connectionType)
    ...
    currentUsageBill = (monthlyCharge.monthlyUsageCharge*usageDays*1.1/30)+(monthlyCharge.monthlyFixedCharge)*2+monthlyMeterRent
    forecastedBillPerCurrentUsage =(monthlyCharge.monthlyUsageCharge*1.1+monthlyCharge.monthlyFixedCharge)*2+monthlyMeterRent
    forecastedBillPerUnderUsage = (monthlyUnderUsageCharge.monthlyUsageCharge*1.1+monthlyUnderUsageCharge.monthlyFixedCharge)*2+monthlyMeterRent
    forecastedBillPerOverUsage =(monthlyOverUsageCharge.monthlyUsageCharge*1.1+monthlyOverUsageCharge.monthlyFixedCharge)*2+monthlyMeterRent

    return {currentUsageBill, forecastedBillPerCurrentUsage, forecastedBillPerUnderUsage, forecastedBillPerOverUsage}
}
...
result()
{
  if(document.getElementById('currentMeterReading')['value']==""){
    ...
    this.averageDailyUsage=(this.currentUnitConsumption/Difference_In_Days).toFixed(2);
    
    let bill= this.calculateBill(this.currentUnitConsumption,Difference_In_Days,connectionType)
    ...
    this.billForecastedWith1UnitReduced= bill.forecastedBillPerUnderUsage.toFixed(2);
    this.billForecastedWith1UnitAdditional= bill.forecastedBillPerOverUsage.toFixed(2);
  }
}
